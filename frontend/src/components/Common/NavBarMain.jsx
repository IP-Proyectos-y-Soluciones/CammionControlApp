...
export function NavBarMain() {
    const { logout, setIsAuthenticated, isAuthenticated, userRole, userName } =
        useAuth();
    ...
    ${menuOpen ? '' : ''}`}
        ...
        >
            <div className="flex items-center space-x-4 ml-4">
                <Link
                    to={handleDashboardLink()}
                    className="text-white flex items-center"
                >
                    <img
                        src={logo}
                        alt="Company Logo"
                        className={`h-[4.2rem] w-auto cursor-pointer transition-transform hover:scale-105 z-30
        ${menuOpen ? 'brightness-150' : ''}`}
            ...
            <div className="hidden md:flex items-center space-x-2 mr-4">
                {isAuthenticated ? (
                    <span className="pt-2 text-white font-bold italic">
                        {userName}
                    </span>
                ) : (
                    <>
            ...
            <div className="flex items-center space-x-4 md:hidden">
                ...
                <button
                    className={`text-white hover:scale-110 ${menuOpen ? 'text-white' : ''} z-30`}
                ...
                {menuOpen && (
                    <>
                        <div
                            className="fixed inset-0 w-full h-full bg-black bg-opacity-80 z-20"
                                    ...
                                    {isAuthorized ? (
                                        <>
                                            <div
                                                ...
                                                className="cursor-pointer pl-4 w-full hover:text-yellow-400"
                                            >
                                                <EmployeeDropdown />
                                            </div>
                                            <div
