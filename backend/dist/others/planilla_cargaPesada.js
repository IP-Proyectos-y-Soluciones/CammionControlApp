...
function plantillaCargaPesada(data) {
  data.forEach(function (item) {
    try {
      ...
      // );
      var nombreArchivo = _path["default"].resolve(__dirname, '../pdf-excel', // Sube un nivel y navega a pdf-excel...
      "Planilla_".concat(n_planilla, ".pdf"));
      var salida = _fs["default"].createWriteStream(nombreArchivo);
      doc.pipe(salida);

      // const logo = path.join(__dirname, './yadiraLogoColor2.png');
      var logo = _path["default"].resolve(__dirname, '../icons', 'yadiraLogoColor2.png');
      doc.image(logo, 50, 3, {
        width: 200
      });
