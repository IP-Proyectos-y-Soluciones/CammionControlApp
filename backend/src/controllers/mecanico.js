...
// Crear un nuevo mecanico
 export const createMecanico = async (req, res) => {
  const { placas, fecha_de_revision, kilometraje, nombre_mecanico, celular, area_de_revision, fecha_entrega, descripcion_revicion } = req.body;
  const mecanico = new Mecanico({ placas, fecha_de_revision, kilometraje, nombre_mecanico, celular, area_de_revision, fecha_entrega, descripcion_revicion });
...
export const updateMecanico = async (req, res) => {
  const { id } = req.params;
  const { placas, fecha_de_revision, kilometraje, nombre_mecanico, celular, area_de_revision, fecha_entrega, descripcion_revicion } = req.body;
  const updates = { placas, fecha_de_revision, kilometraje, nombre_mecanico, celular, area_de_revision, fecha_entrega, descripcion_revicion };
  
  // Filtrar las propiedades no definidas
  Object.keys(updates).forEach(key => updates[key] === undefined && delete updates[key]);

  try {
    const mecanico = await Mecanico.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
    if (!mecanico) {
