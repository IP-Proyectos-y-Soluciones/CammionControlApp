...
export const createVehiculo = async (req, res) => {
  const { placa, tipo_de_combustible, clase_de_vehiculo, marca, color, propietario, documentos, volquetas, tanqueos } = req.body;
  const vehiculo = new Vehiculo({ placa, tipo_de_combustible, clase_de_vehiculo, marca, color, propietario, documentos, volquetas, tanqueos });
...
export const updateVehiculo = async (req, res) => {
  const { id } = req.params;
  const { placa, tipo_de_combustible, clase_de_vehiculo, marca, color, propietario, documentos, volquetas, tanqueos } = req.body;
  const updates = { placa, tipo_de_combustible, clase_de_vehiculo, marca, color, propietario, documentos, volquetas, tanqueos };
  
  // Filtrar las propiedades no definidas
  Object.keys(updates).forEach(key => updates[key] === undefined && delete updates[key]);

  try {
    const vehiculo = await Vehiculo.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
    if (!vehiculo) {
