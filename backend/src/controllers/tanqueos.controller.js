...
export const createTanqueo = async (req, res) => {
  ...
  const { fecha_tanqueo, n_recibo, estacion, cantidad_galones, valor_tanqueo, vehiculo, conductor } = req.body;
  ...
  try {
    ...
    const tanqueo = new Tanqueo({ fecha_tanqueo, n_recibo, estacion, cantidad_galones, valor_tanqueo, vehiculo:vehiculoById._id, conductor:persona._id });
    await tanqueo.save();
    res.status(201).json({
...
export const updateTanqueo = async (req, res) => {
  const { id } = req.params;
  const { fecha_tanqueo, n_recibo, estacion, cantidad_galones, valor_tanqueo, vehiculo, conductor } = req.body;
  const updates = { fecha_tanqueo, n_recibo, estacion, cantidad_galones, valor_tanqueo, vehiculo, conductor };
  
  // Filtrar las propiedades no definidas
  Object.keys(updates).forEach(key => updates[key] === undefined && delete updates[key]);

  try {
    const tanqueo = await Tanqueo.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
    if (!tanqueo) {
      return res.status(404).json();
    }
    res.status(200).json({
